...
//	   pm.makeLineAndShapeChart(); 
   ...
   */ 
   ...
   /**
	 ...
	 */
	public String createBarChart(CategoryDataset dataset, String xName,
			String yName, String chartTitle, String charName,String filePath) {
		JFreeChart chart = ChartFactory.createBarChart(chartTitle, // 图表标题
				xName, // 目录轴的显示标签
				yName, // 数值轴的显示标签
				dataset, // 数据集
		...
		TextTitle t = chart.getTitle();
		   t.setFont(labelFont);//标题文字

		domainAxis.setLabelFont(labelFont);//x轴标题文字
		domainAxis.setTickLabelFont(labelFont);//x轴坐标上文字
		vn.setLabelFont(labelFont);//y轴标题文字
		vn.setTickLabelFont(labelFont);//y轴坐标上文字
		chart.getLegend().setItemFont(labelFont);//图例文字
		...
		try {
			...
			String chartName = filePath + File.separator +charName;
			fos_jpg = new FileOutputStream(chartName);
			ChartUtilities.writeChartAsPNG(fos_jpg, chart, 800, 300, true, 10);
			return chartName;
		} catch (Exception e) {
			e.printStackTrace();
			return null;
