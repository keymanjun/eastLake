...
$.funkyUI   = function(opts) { install(window, opts); };
$.unfunkyUI = function(opts) { remove(window, opts); };
...
function install(el, opts) {
    ...
    var z = opts.baseZ;
		...
		//如果当前窗口是主窗口则计算窗口的高宽和滚动条的位置
		if (window==window.parent){
			//计算窗口的显示位置,确保显示在屏幕正中间
	    $.DialogData.cw = $.boxModel ?$(window).width():$('body').width();//窗体高度
			$.DialogData.ch = $.boxModel ?$(window).height():$('body').height();//窗体宽度//必须考虑文本框处于页面边缘处，控件显示不全的问题
			$.DialogData.sw = Math.max($(document).scrollLeft(), $('body').scrollLeft());//横向滚动条位置
			$.DialogData.sh = Math.max($(document).scrollTop(), $('body').scrollTop());//纵向滚动条位置//考虑滚动的情况
		}
    ...
    var arr = [];
		...
		arr.push("           	<input id='_ButtonCancel_"+opts.id+"'  type='button' onclick=\"$.unfunkyUI('"+opts.id+"');\" value='取 消'>");
		arr.push("            </div></td></tr>");
		arr.push("      </table></td>");
		...
		var lyr2 = $("#_DialogBGDiv",topWin.document.body);
		if(!lyr2.length>0){
			lyr2 = $('<div class="funkyUI blockOverlay" style="z-index:'+ z++ +';cursor:wait;border:none;margin:0;padding:0;width:100%;height:100%;top:0;left:0"></div>');
			lyr2.attr("id","_DialogBGDiv");
			$(lyr2[0]).appendTo(full ? 'body' : el);
			$.DialogData.backgroundDiv = lyr2;
		}else{
			topWin.$.DialogData.backgroundDiv.css('z-index',parseInt(window.parent.$.DialogData.dialogDiv.css('z-index'))+1);	
			z = parseInt(topWin.$.DialogData.backgroundDiv.css('z-index'))+1;	
		}
		var lyr3 = $("#_DialogDiv_"+opts.id,topWin.document.body);
		if(!lyr3.length>0){
			lyr3 = full ? $('<div class="funkyUI blockMsg blockPage" style="z-index:'+ z +';position:fixed"></div>',topWin.document.body)
	                    : $('<div class="funkyUI blockMsg blockElement" style="z-index:'+ z +';display:none;position:absolute">'+opts.message+'</div>',topWin.document.body);
			lyr3.hide();
    ...
    if(full){
	    ...
	    lyr3.css({left:css.left+"px",top:css.top+"px",width:css.width+"px",height:css.height+"px"});
			//记录当前弹出窗口的iframe对象
			$.DialogData.iframeObj = $("#_DialogFrame_"+opts.id,topWin.document.body);
...
function remove(el, opts) {
    ...
    if (full) {
    	if(topWin.$._Array.length>1){
    		...
    		topWin.$.DialogData.backgroundDiv.css('z-index',parseInt(window.parent.$.DialogData.dialogDiv.css('z-index'))-1);	
    		window.parent.$.DialogData.dialogDiv.hide(); 
    		window.parent.$.DialogData.dialogDiv.fadeIn("fast"); 
    		win.$.DialogData.dialogDiv.fadeOut("normal"); 
    		bind(0, win, opts); // unbind events
    	}else{
    		topWin.$._Array.length = 0;
    		window.$.DialogData.dialogDiv.hide();
    		window.$.DialogData.dialogDiv.html();
    		bind(0, el, opts); // unbind events
    		var els = $('body').children().filter('.funkyUI') ;
    		if (opts.fadeOut) {
...
function bind(b, el, opts) {
    ...
    b ? $(document).bind(events, {opts:opts,el:el}, handler) : $(document).unbind(events, handler);
