...
dTree.prototype.node = function(node, nodeId) {
  ...
  if(node.pid==-1){
  	str+='<img src=" '+ this.icon.root +'"/>';
  	  ...
  	  /**2010-03-01 胡俊修改(将checkbox改为radio功能)**/
	  //新增checkbox/radio表单元素
  	  var strType = 'type="checkbox" class="checkbox" onclick=tree.doChecked("'+nodeId+'")';
  	  if(this.config.radio==true) strType='type="radio" name="c'+  this.obj.name + '" onclick=tree.showNodeInfo("'+node.name+'","'+actionvalue+'","' +this.obj.name + nodeId +'")';
	  str+= '<input '+strType+' id="c'+  this.obj.name + nodeId + '" ';
	  
	  //str+= ' value="'+node.name+'" onclick="'+this.obj.name+'.doClick('+nodeId+')"/>';
	  str+= ' value="'+node.id+'" pcode="'+node.pid+'">';
  }
  //str += node.title;
	str += '<a href="#" onclick=tree.showNodeInfo("'+node.name+'","'+actionvalue+'","' +this.obj.name + nodeId +'")>'+node.title+'</a>'
	str += '</div>';
...
dTree.prototype.showNodeInfo=function(nodeId, actionvalue, nodename){
	/**2010-03-01 胡俊修改(将checkbox改为radio功能)**/
	var radioObj = document.getElementById("c"+ nodename);
	if(radioObj) radioObj.checked=true;
		...
		if(nodeId==''){
			...
			array = nodeId.split(",");
	   		...
	   		if(nodeId==""){
				...
				window.parent.document.getElementById('shownodeinfo').src=contextpath+"/goodsSortPropAction!queryNodeProp.action?code="+nodeId; 
	 		}
		}
	}else if(actionvalue=="/goodsSortBrandAction.action1"){
		/**2010-03-01 胡俊修改(屏蔽自定义属性功能)**/
		//window.parent.document.getElementById('treeinfo').src=contextpath+"/goodsSortBrandAction!executeBrand.action?nodeId="+nodeId;
		window.parent.document.getElementById('treeinfo').src=contextpath+"/goodsSortPropAction!addNodeInfo.action?parentno="+nodeId+"&opertype=1";
	}else if(actionvalue=="/goodsSortBrandAction.action2"){
		window.parent.document.getElementById('treeinfo').src=contextpath+"/goodsSortBrandAction!executeGoods.action?nodeId="+nodeId;
	}
}
