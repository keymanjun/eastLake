...
function setday(tt,obj) //主调函数
{
	...
	outObject = (arguments.length == 1) ? th : obj;
	outButton = (arguments.length == 1) ? null : th;	//设定外部点击的按钮
// add by tjchen for szAir ssms-----
if(parent.document.all['dynFrame']){
parent.document.all['dynFrame'].style.height=this.document.body.scrollHeight+200;
}
//----------------------------------
	//根据当前输入框的日期显示日历的年月
	var reg = /^(\d+)-(\d{1,2})-(\d{1,2})/;		//不含时间
...
parent.document.all['mainFrame'].style.height=this.document.body.scrollHeight+200;
}
//----------------------------------
	//根据当前输入框的日期显示日历的年月
	var reg = /^(\d+)-(\d{1,2})/;		//不含时间
	var r = outObject.value.match(reg);	
	if(r!=null){
		r[2]=r[2]-1; 
